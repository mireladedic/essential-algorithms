...
template<typename Tip>
class AgrLStek : private JednostrukaLista<Tip>{
public:
    bool JeLiPrazan(){
        return JednostrukaLista<Tip>::JeLiPrazna();
    }

    void StaviNaStek(const Tip &info){
        JednostrukaLista<Tip>::Umetni(info);
    }

    Tip SkiniSaSteka(){
        return JednostrukaLista<Tip>::Izbaci();
    }

    const Tip &ElementNaVrhu() const {
        return JednostrukaLista<Tip>::DajTrenutniElement();
    }

    void Brisi(){
        JednostrukaLista<Tip>::Brisi();
    }

    int Duzina()const{
        return JednostrukaLista<Tip>::DDuzina() + JednostrukaLista<Tip>::LDuzina();
    }

    void Prikazi()const{
        JednostrukaLista<Tip>::Prikazi();
    }
};
int main()
{
